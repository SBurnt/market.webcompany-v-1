...
function localStorage_filter_get(localStorage_name, block_name, block_status) {
    if (localStorage.getItem(localStorage_name)) {
        let j_filter_arrow = localStorage.getItem(localStorage_name);
        var o_filter_arrow = JSON.parse(j_filter_arrow);
    } else {
        var o_filter_arrow = {};
    }

    o_filter_arrow[block_name] = block_status;
    let filter_update = JSON.stringify(o_filter_arrow);
    localStorage.setItem(localStorage_name, filter_update);
}

//Занесение данных по кликам на кнопку сворачивания блоков
function filter_arrow_localStorage(element) {
    let block_name = element.closest('.filter-section').find('.filter__title').attr('class');
    let tv_name_fltr = block_name.split(' ');
    let tv_block_name = tv_name_fltr[tv_name_fltr.length - 1];
    let block_status = element.closest('.filter-section').hasClass('active');
    //console.log(tv_block_name,block_status);

    localStorage_filter_get('filter_arrow', tv_block_name, block_status);
}

function filter_more_localStorage(element) {
    let block_name = element.closest('.filter-section').find('.filter__title').attr('class');
    let tv_name_fltr = block_name.split(' ');
    let tv_block_name = tv_name_fltr[tv_name_fltr.length - 1];
    let block_status = element.hasClass('show');
    console.log(tv_block_name, block_status);
    localStorage_filter_get('filter_more', tv_block_name, block_status);
}
...
function asideFilterSlideToggle() {
    ...
    $(document).on('click', '.filter-arrow', function () {
        ...
        //Заносим данные об открытых блоках в localStorage
        filter_arrow_localStorage(click_element);
...
//Сворачиваем или разворачиваем блоки в фильтре
function filter_arrow_more_init() {

    //Скрытие блока
    if (localStorage.getItem('filter_arrow')) {
        let j_filter_arrow = localStorage.getItem('filter_arrow');
        let o_filter_arrow = JSON.parse(j_filter_arrow);
        if (o_filter_arrow != undefined && Object.keys(o_filter_arrow).length > 0) {
            for (key in o_filter_arrow) {
                let class_name = '.filter__title.' + key;
                let block_status = $(class_name).closest('.filter-section').hasClass('active');
                if (block_status == o_filter_arrow[key]) {
                    $(class_name).closest('.filter__header').find('.filter-arrow').click();
                }
            }
        }
    }

    //Разворачивание полей с блольшим количеством эле
    if (localStorage.getItem('filter_more')) {
        let j_filter_more = localStorage.getItem('filter_more');
        let o_filter_more = JSON.parse(j_filter_more);
        if (o_filter_more != undefined && Object.keys(o_filter_more).length > 0) {
            for (key in o_filter_more) {
                let re = /name/gi;
                let replace_key = key.replace(re, 'block');
                let class_name = '.' + replace_key + ' .filter-section__more';
                let block_status = $(class_name).hasClass('show');
                if (block_status != o_filter_more[key]) {
                    $(class_name).click();
                }
            }
        }
